...
Three results emerge from numerical experiments at $N = 12$--$20$
(Hilbert space dimension up to $2^{20} = 1{,}048{,}576$).
...
with Pearson $r$ stable at $-0.57$ from $N=12$ to $N=20$
(35 Hamiltonians across four system sizes, all negative,
per-seed standard deviation collapsing from $0.22$ to $0.06$).
...
(Papers~I--III use Gaussian couplings $J_{ij} \sim \mathcal{N}(0,1)$;
the switch to uniform couplings with independent per-channel
coefficients tests universality across ensemble choices).
Ground states are computed by exact diagonalization ($N = 12$,
Hilbert space dimension $4096$) or sparse Lanczos ($N = 14$--$20$,
up to dimension $1{,}048{,}576$).
...
\paragraph{Confirmation at $N = 20$.}
At $N = 20$ (Hilbert space dimension $1{,}048{,}576$), the
full MI graph has $\sim$95 edges after pruning.  Across 5
ground states, the mean edge-level correlation is
$r(\kappa, T_\mathrm{ZZ}) = -0.43 \pm 0.14$, with
partial $r = -0.36 \pm 0.13$ after degree control.
Four of five seeds are individually significant at
$p < 0.01$.  The coupling is unchanged from $N = 12$:
this is not a finite-size effect.
...
Across all system sizes tested ($N = 12$--$20$),
$T_\mathrm{obs}$ and $S_\mathrm{obs}$ are strongly
anti-correlated.  The effect is present in \emph{every}
Hamiltonian tested (35 out of 35).  At $N = 20$
(Hilbert space dimension $1{,}048{,}576$), the mean
correlation is $r = -0.57$ with a per-seed standard
deviation of just $0.06$---the effect has stabilized
and become highly reproducible
(Table~\ref{tab:complementarity}).
Quarter-system observers ($k = 5$, $k/N = 0.25$) at
$N = 20$ see $r = -0.72 \pm 0.11$, confirming the
partiality dependence at scale.
...
\caption{Complementarity strength $r(T_\mathrm{obs}, S_\mathrm{obs})$
versus system size $N$ for half-system observers.  The mean
stabilizes near $r \approx -0.6$ while the per-seed variance
collapses from $0.22$ ($N = 12$) to $0.06$ ($N = 20$),
confirming this is not a finite-size artifact.
(Note: Fig.\ shows $N = 12$--$16$; $N = 20$ data in
Table~\ref{tab:complementarity}.)}
\label{fig:scaling}
\end{figure}
...
the edge-level variance at $N = 12$ and is unchanged
at $N = 20$ ($r = -0.43$, $\sim$95 edges per graph).
The remaining variance is expected from the stochastic
nature of curvature on finite graphs and
from quantum fluctuations in the correlations.
The stability across system sizes suggests
the coupling is a structural property, not a finite-size
artifact.
...
Three directions follow.  First, extending to $N = 24$--$28$
will test whether the variance collapse continues and
whether the complementarity asymptotes.  Second, defining a discrete
...
$r = -0.44$, independent of node degree, stable from
$N = 12$ to $N = 20$.  Observer by observer, stress-energy
and entanglement entropy anti-correlate at $r \approx -0.57$
(half-system) to $-0.72$ (quarter-system), with per-seed
variance collapsing from $0.22$ to $0.06$ as $N$ grows.
The effect is universal across system sizes, Hamiltonians,
...
J.~Glass, ``Emergent Spatial Locality from Partial Observation in Finite Quantum Systems,''
Zenodo, DOI:~10.5281/zenodo.18883867 (2026).

\bibitem{glass2026plc2}
J.~Glass, ``Emergent Curvature from Partial Observation in Finite Quantum Systems,''
