#!/usr/bin/env python3
"""
Extended topology k-sweep WITH distance floor.

Same as run_n20_topology_extended.py but uses the 5th-percentile
minimum-distance floor in the ORC computation to eliminate the 1/epsilon
instability at small k.

10 Hamiltonians × 20 subsets = 200 trials per k.
"""
...
# min_distance=None uses 5th-percentile floor (default)
MIN_DISTANCE = None

print("=" * 60)
print(f"  EXTENDED TOPOLOGY K-SWEEP WITH DISTANCE FLOOR (N={N})")
print(f"  {N_SEEDS} Hamiltonians × {N_SUBSETS} subsets = {N_SEEDS * N_SUBSETS} trials/k")
print(f"  k values: {K_VALUES}")
print(f"  Distance floor: 5th percentile of 1/MI")
print("=" * 60)
...
# Summary
print(f"\n{'='*60}")
print(f"  RESULTS WITH DISTANCE FLOOR ({N_SEEDS * N_SUBSETS} trials per k)")
print(f"{'='*60}")
...
outpath = RESULTS_DIR / 'n20_exp3_topology_k_sweep_floored.json'
with open(outpath, 'w') as f:
    json.dump({
        'experiment': 'N=20 Exp3 Topology k-sweep (extended, distance floor)',
        'N': N, 'k_values': K_VALUES,
        'seeds': SEEDS, 'n_subsets': N_SUBSETS,
        'n_trials_per_k': N_SEEDS * N_SUBSETS,
        'threshold': THRESHOLD,
        'distance_floor': '5th percentile of 1/MI per graph',
        'summary': summary,
        'trials': all_trials,
